            ...
            while not self.shutdown_event.is_set():
                ...
                for payload in messages:
                    if self.executor_fn:
                        ...
                        await loop.run_in_executor(
                            self.executor, self.log, f"{addr} | {payload!r}"
                        )
...
async def main(port, log_file, debug):
    server = TCPServer(port=port, log_file=log_file, debug=debug)
    ...
    try:
        loop.add_signal_handler(
            signal.SIGINT, lambda: asyncio.create_task(server.shutdown())
        )

        loop.add_signal_handler(
            signal.SIGTERM, lambda: asyncio.create_task(server.shutdown())
        )
...
if __name__ == "__main__":
    ...
    # Subparser server
    server = sub.add_parser("server", help="Run in server mode")
    ...
    server.add_argument("--log", "-l", help="Name of server log file", default=None)
    server.add_argument(
        "--port",
        "-p",
        help="TCP Port",
    ...
    # Subparser client
    client = sub.add_parser("client", help="Run in client mode")
    ...
    if args.mode == "server":
        ...
        try:
            asyncio.run(main(port=args.port, log_file=args.log, debug=args.debug))
